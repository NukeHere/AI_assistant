...
- создаёт постоянный уникальный `client_id` для этой установки/устройства, показывает его в приложении и позволяет скопировать `/link <client_id>` для Telegram;
...
7. Проверь поле `Client ID`: можно оставить сгенерированный ID или вставить нужный ID профиля, если хочешь привязать телефон к уже существующей памяти.
