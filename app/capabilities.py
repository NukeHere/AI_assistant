...
def capabilities_answer(persona: Persona) -> str:
    if persona == Persona.ALIEN:
        return (
            ...
            "5. Синхронизация: компьютер и Android обмениваются событиями через серверную реку `/v1/sync`, чтобы ноты не исчезали при перезапуске.\n"
            "6. Резервные слои: snapshot `/v1/snapshot` и локальный backup-agent могут сохранять копии памяти и диалогов.\n"
            "7. Оформление контакта: могу использовать Observation, Diagnosis, Recommended action, Command action, Explanation и Conclusion, когда это усиливает ясность или атмосферу.\n"
    ...
    return (
        ...
        "5. Синхронизация: desktop и Android обмениваются событиями через `/v1/sync`, поэтому история должна объединяться, а не затираться.\n"
        "6. Бэкапы: `/v1/snapshot` и локальный backup-agent сохраняют резервные копии памяти и истории.\n"
        "7. Оформление ответов: могу использовать Observation, Diagnosis, Recommended action, Command action, Explanation и Conclusion, если это полезно.\n"
