...
```assistant_memory
{"telegram_action":{"reply_to":"group|private|both","mention_sender":true,"group_text":"необязательный короткий ответ в группу","private_text":"необязательный личный ответ","private_to_username":"username_без_@","private_to_telegram_user_id":"123456789","reaction_emoji":"👍"}}
```
Используй reply_to="private" для конфиденциальной или персональной информации. По умолчанию private отправляется автору сообщения. Если пользователь просит написать другому человеку, обязательно укажи private_to_username или private_to_telegram_user_id и private_text — не отправляй это автору запроса. Бот сможет написать только тем, чей Telegram id уже известен или кто раньше писал боту. Используй mention_sender=true, когда групповой ответ должен явно обратиться к отправителю. Если вместо текста достаточно реакции на сообщение пользователя, добавь reaction_emoji и не дублируй это видимым текстом: сервер подавит обычный ответ для reaction-only действия. Если нужен и текст, и реакция, явно задай group_text/private_text или reply_to="both". Никогда не помещай секреты в telegram_action.
""".strip()
