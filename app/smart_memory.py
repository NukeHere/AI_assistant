...
```
Используй reply_to="private" для конфиденциальной или персональной информации. Используй mention_sender=true, когда групповой ответ должен явно обратиться к отправителю. Если вместо текста достаточно реакции на сообщение пользователя, добавь reaction_emoji и не дублируй это видимым текстом: сервер подавит обычный ответ для reaction-only действия. Если нужен и текст, и реакция, явно задай group_text/private_text или reply_to="both". Никогда не помещай секреты в telegram_action.
""".strip()
