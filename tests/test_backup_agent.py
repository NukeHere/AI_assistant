...
class BackupAgentTests(unittest.TestCase):
    ...
    def test_sync_backup_restores_when_remote_lost_telegram_links(self) -> None:
        calls: list[dict[str, object]] = []

        def fake_request(url: str, token: str, payload: dict[str, object], timeout: int = 60) -> dict[str, object]:
            calls.append(payload)
            if payload["action"] == "export":
                return {"snapshot": {"client_id": "primary-user", "conversations": [{"id": "default"}], "messages": [{"content": "fresh"}], "memories": [], "memory_cells": [], "timed_memories": [], "telegram_users": []}}
            return {"ok": True, "imported": {"telegram_users": 1}}
        ...
        try:
            with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temp_dir:
                ...
                latest.write_text(
                    '{"format":"ai-assistant-snapshot-v1","client_id":"primary-user","conversations":[],"messages":[],"memories":[],"memory_cells":[],"timed_memories":[],"telegram_users":[{"telegram_user_id":"777","guest_client_id":"tg-guest","app_client_id":"primary-user","is_authorized":1}]}',
                    encoding="utf-8",
                )
                action, path = backup_agent.sync_backup("https://example.com/v1/snapshot", "token", "primary-user", backup_dir, keep=10)
        finally:
    ...
    def test_sync_backup_preserves_remote_telegram_links_when_local_is_stale(self) -> None:
        imports: list[dict[str, object]] = []

        def fake_request(url: str, token: str, payload: dict[str, object], timeout: int = 60) -> dict[str, object]:
            if payload["action"] == "export":
                return {"snapshot": {"client_id": "primary-user", "conversations": [], "messages": [], "memories": [], "memory_cells": [], "timed_memories": [], "telegram_users": [{"telegram_user_id": "777", "guest_client_id": "tg-guest", "app_client_id": "primary-user", "is_authorized": 1}]}}
            imports.append(payload)
            return {"ok": True, "imported": {"messages": 1, "telegram_users": 1}}
        ...
        try:
            with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temp_dir:
                ...
                latest.write_text(
                    '{"client_id":"primary-user","conversations":[{"id":"default"}],"messages":[{"content":"old"},{"content":"more"}],"memories":[],"memory_cells":[],"timed_memories":[],"telegram_users":[]}',
                    encoding="utf-8",
                )
                action, path = backup_agent.sync_backup("https://example.com/v1/snapshot", "token", "primary-user", backup_dir, keep=10)
        finally:
        ...
        self.assertEqual(action, "restored")
        self.assertEqual(path, latest)
        imported_snapshot = imports[-1]["snapshot"]
        self.assertEqual(len(imported_snapshot["telegram_users"]), 1)

    def test_restore_backup_preserves_remote_telegram_links_when_file_is_stale(self) -> None:
        imports: list[dict[str, object]] = []
        ...
        try:
            with tempfile.TemporaryDirectory(ignore_cleanup_errors=True) as temp_dir:
                ...
                path.write_text('{"client_id":"primary-user","conversations":[],"messages":[{"content":"old"}],"memories":[],"memory_cells":[],"timed_memories":[],"telegram_users":[]}', encoding="utf-8")
                backup_agent.restore_backup("https://example.com/v1/snapshot", "token", "primary-user", path)
        finally:
            backup_agent.request_snapshot = old_request
